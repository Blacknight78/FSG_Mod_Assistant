...
const modHub = {
	...
	refresh : () => {
		...
		if ( net.isOnline() ) {
			...
			const completeRequest = (label, result) => {
				if ( completedRequests.has(label) ) { return }
				completedRequests.add(label)
				pendingRequests -= 1
				if ( pendingRequests === 0 ) {
					serveIPC.log.info('performance', `ModHub refresh completed in ${(performance.now() - refreshStartedAt).toFixed(1)} ms last=${label}:${result}`)
				}
			}
			...
			request_22.on('response', (response) => {
				...
				response.on('end',  () => {
					...
					modHub.load()
					completeRequest('FS22', response.statusCode.toString())
				})
			})
			request_22.on('abort', () => {
				modHub.load()
				modHub.log.info('Network abort')
				completeRequest('FS22', 'abort')
			})
			request_22.on('error', (err) => {
				modHub.load()
				modHub.log.info('Network error', err)
				completeRequest('FS22', 'error')
			})
			request_22.end()
			...
			request_25.on('response', (response) => {
				...
				response.on('end',  () => {
					...
					modHub.load()
					completeRequest('FS25', response.statusCode.toString())
				})
			})
			request_25.on('abort', () => {
				modHub.load()
				modHub.log.info('Network abort')
				completeRequest('FS25', 'abort')
			})
			request_25.on('error', (err) => {
				modHub.load()
				modHub.log.info('Network error', err)
				completeRequest('FS25', 'error')
			})
			request_25.end()
		} else {
			serveIPC.log.info('performance', 'ModHub refresh skipped; network offline')
		}
	},
}
