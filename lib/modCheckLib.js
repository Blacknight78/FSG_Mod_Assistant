...
class modFileCollection {
	...
	#modHubRecentIDs = new Set()
	#modHubMatchCache = new Map()
	#savedModHubIDsByNormalizedName = null
	...
	set modHubList(newList)    {
		...
		this.#modHubRecentIDs = new Set((newList?.last ?? []).map((modHubID) => parseInt(modHubID)))
		this.#modHubMatchCache = new Map()
	...
	modHubFullRecord(thisMod) {
		const shortName = thisMod.fileDetail.shortName
		const match = this.#modHubMatchCache.get(shortName) ?? this.modHubMatchShortName(shortName)
		if ( !this.#modHubMatchCache.has(shortName) ) { this.#modHubMatchCache.set(shortName, match) }
		...
		return {
			...
			id      : modHubID,
			match   : match.method,
			recent  : modHubID !== null && this.#modHubRecentIDs.has(parseInt(modHubID)),
			version : this.modHubVersionModHubId(modHubID),
		}
	}

	modHubMatchShortName(shortName, savedIDsByNormalizedName = null) {
		...
		const savedIDMap = savedIDsByNormalizedName instanceof Map ? savedIDsByNormalizedName : this.#getSavedModHubIDsByNormalizedName()
		const savedIDs = savedIDMap.get(normalizedName) ?? new Set()
		if ( savedIDs.size === 1 ) {
	...
	#getSavedModHubIDsByNormalizedName() {
		if ( this.#savedModHubIDsByNormalizedName !== null ) { return this.#savedModHubIDsByNormalizedName }

		const records = serveIPC.storeLibrary?.get?.('records', {}) ?? {}

		this.#savedModHubIDsByNormalizedName = new Map()
		for ( const record of Object.values(records) ) {
			const modHubIDs = record?.modHubIDs ?? []
			if ( !Array.isArray(modHubIDs) || modHubIDs.length === 0 ) { continue }
			const modNames = Array.isArray(record?.modNames) ? record.modNames : []

			for ( const name of [record?.fileName, ...modNames] ) {
				const normalizedName = normalizeModHubName(name)
				if ( normalizedName === '' ) { continue }
				this.#savedModHubIDsByNormalizedName.set(
					normalizedName,
					new Set([...(this.#savedModHubIDsByNormalizedName.get(normalizedName) ?? []), ...modHubIDs])
				)
			}
		}
		return this.#savedModHubIDsByNormalizedName
	}
	...
	clearAll() {
		...
		this.#savedModHubIDsByNormalizedName = null
		this.#modHubMatchCache.clear()
		this.#loadingProgressPending         = 0
		this.#resetScanStats()
	}
	...
	#resetScanStats() {
		this.#scanStats = {
			...
			dispatchMS      : 0,
			fileStatsMS     : 0,
			folders         : this.#set_Collections.size,
			modsAddCloneMS  : 0,
			modsAddDependMS : 0,
			modsAddedMS     : 0,
			modsAddIndexMS  : 0,
	...
	#logScanPerformance(label, startedAt, extraDetail = '') {
		const detailText = extraDetail === '' ? '' : ` ${extraDetail}`
		this.#log.info('performance', `${label} took ${(performance.now() - startedAt).toFixed(1)} ms${detailText}`)
	}
	...
	addCollection(folder, offline = false) {
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.collectionFiles += goodFolderContents.length
		}
		this.#logScanPerformance('Mod folder collection read', startedAt, `folder=${thisShortName} files=${goodFolderContents.length.toString()} offline=${offline.toString()}`)
	...
	#ensureThreads(threadTarget = 1) {
		const targetThreadCount = Math.max(1, Math.min(this.#threadPoolMax, threadTarget))
		if ( this.#threadPoolCount >= targetThreadCount ) { return }
		if ( this.#scanStats !== null ) { this.#scanStats.threadsStarted = true }
		if ( this.#threadPoolCount === 0 ) {
			this.#threadPool.length = 0
			this.#threadPoolIdx.clear()
			this.#threadCurrent = 0
		}

		for ( let idx = this.#threadPool.length + 1; idx <= targetThreadCount; idx++ ) {
	...
	async processMods() {
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.dispatchMS = performance.now() - dispatchStartedAt
		}
		if ( this.#threadPoolCount === 0 ) {
			this.processModsPost()
			this.#threadDoneEmit.emit('process-mods-done')
	...
	processModsPost() {
		...
		if ( this.#list_newMods.size !== 0 ) {
			this.#modCache.saveFile()
		}
		const sortStartedAt = performance.now()
		this.#doAlphaSort()
		if ( this.#scanStats !== null ) { this.#scanStats.postSortMS = performance.now() - sortStartedAt }
		const conflictsStartedAt = performance.now()
		this.#processBindConflicts()
		if ( this.#scanStats !== null ) { this.#scanStats.postConflictsMS = performance.now() - conflictsStartedAt }
		const requiredStartedAt = performance.now()
		this.#doRequired()
		if ( this.#scanStats !== null ) { this.#scanStats.postRequiredMS = performance.now() - requiredStartedAt }
		const badgesStartedAt = performance.now()
		this.#doBadges()
		if ( this.#scanStats !== null ) { this.#scanStats.postBadgesMS = performance.now() - badgesStartedAt }
		if ( this.#scanStats !== null ) {
			this.#scanStats.postProcessMS = performance.now() - startedAt
			this.#log.info('performance', [
				'Mod folder scan internals',
				`files=${this.#scanStats.totalFiles.toString()}`,
				`collectionFiles=${this.#scanStats.collectionFiles.toString()}`,
				`cacheHits=${this.#scanStats.cacheHits.toString()}`,
				`cacheMisses=${this.#scanStats.cacheMisses.toString()}`,
				`newCachedMods=${this.#list_newMods.size.toString()}`,
				`threadsStarted=${this.#scanStats.threadsStarted.toString()}`,
				`dispatch=${this.#scanStats.dispatchMS.toFixed(1)} ms`,
				`fileStats=${this.#scanStats.fileStatsMS.toFixed(1)} ms`,
				`cacheLookup=${this.#scanStats.cacheLookupMS.toFixed(1)} ms`,
				`modsAdded=${this.#scanStats.modsAddedMS.toFixed(1)} ms`,
				`progressUpdates=${this.#scanStats.progressUpdates.toString()}`,
				`postProcess=${this.#scanStats.postProcessMS.toFixed(1)} ms`,
			].join(' '))
			this.#log.info('performance', [
				'Mod folder add-mod internals',
				`clone=${this.#scanStats.modsAddCloneMS.toFixed(1)} ms`,
				`modHub=${this.#scanStats.modsAddModHubMS.toFixed(1)} ms`,
				`indexes=${this.#scanStats.modsAddIndexMS.toFixed(1)} ms`,
				`depends=${this.#scanStats.modsAddDependMS.toFixed(1)} ms`,
				`progress=${this.#scanStats.modsAddProgressMS.toFixed(1)} ms`,
			].join(' '))
			this.#log.info('performance', [
				'Mod folder post-process internals',
				`sort=${this.#scanStats.postSortMS.toFixed(1)} ms`,
				`conflicts=${this.#scanStats.postConflictsMS.toFixed(1)} ms`,
				`required=${this.#scanStats.postRequiredMS.toFixed(1)} ms`,
				`badges=${this.#scanStats.postBadgesMS.toFixed(1)} ms`,
			].join(' '))
		}
	}
	...
	#cacheHit(thisFileStats) {
		if ( this.#skipCache || thisFileStats.hashString === null ) {
			return false
		}

		const currentCacheRecord = this.#modCache.getMod(thisFileStats.hashString)
		if ( currentCacheRecord !== false ) { return currentCacheRecord }

		if ( thisFileStats.legacyHashString === null ) { return false }

		const legacyCacheRecord = this.#modCache.getMod(thisFileStats.legacyHashString)
		if ( legacyCacheRecord === false ) { return false }

		const cachedFileTime = Date.parse(legacyCacheRecord?.fileDetail?.fileDate ?? '')
		if ( Number.isNaN(cachedFileTime) ) { return false }

		const currentFileTime = thisFileStats.modifiedDate.getTime()
		return Math.abs(cachedFileTime - currentFileTime) < 1000 ? legacyCacheRecord : false
	}

	#folderStats(folderPath) {
		const pending = [{ displayPath : '', fullPath : folderPath }]
		const signatureParts = []
		let totalSize = 0

		while ( pending.length !== 0 ) {
			const currentFolder = pending.pop()
			for ( const entry of fs.readdirSync(currentFolder.fullPath, { withFileTypes : true }) ) {
				if ( this.#junkRegex.test(entry.name) ) { continue }
				const entryDisplayPath = path.join(currentFolder.displayPath, entry.name)
				const entryFullPath = path.join(currentFolder.fullPath, entry.name)
				const entryStats = fs.statSync(entryFullPath)
				if ( entry.isDirectory() ) {
					pending.push({ displayPath : entryDisplayPath, fullPath : entryFullPath })
					signatureParts.push(`d:${entryDisplayPath}:${entryStats.ctimeMs}`)
				} else {
					totalSize += entryStats.size
					signatureParts.push(`f:${entryDisplayPath}:${entryStats.size}:${entryStats.mtimeMs}`)
				}
			}
		}
		signatureParts.sort(Intl.Collator().compare)
		return {
			hashString : this.#getMD5Hash(signatureParts.join('|')),
			size       : totalSize,
		}
	}
	...
	#fileStats(collectKey, thisFile) {
		...
		try {
			if ( thisFile.isSymbolicLink() ) {
				...
				fileStats.modifiedDate = thisSymLinkStat.mtime
				...
				fileStats.modifiedDate = theseStats.mtime

				if ( !fileStats.isFolder ) { fileStats.size = theseStats.size }
			}

			if ( fileStats.isFolder ) {
				const folderStats = this.#folderStats(isSymLink !== null ? isSymLink : fullPath)
				fileStats.size = folderStats.size
				fileStats.hashString = this.#getMD5Hash(`${thisFile.name}-${folderStats.hashString}`)
			} else {
				fileStats.hashString = this.#getMD5Hash(`${thisFile.name}-${fileStats.size}-${fileStats.modifiedDate.toISOString()}`)
				fileStats.legacyHashString = this.#getMD5Hash(`${thisFile.name}-${fileStats.size}-${fileStats.birthDate.toISOString()}`)
			}
		} catch (err) {
			...
			fileStats.modifiedDate = new Date(1969, 1, 1, 0, 0, 0, 0)
	...
	#addModToData(collectKey, modRecord) {
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddCloneMS += performance.now() - cloneStartedAt
		}
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddModHubMS += performance.now() - modHubStartedAt
		}
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddIndexMS += performance.now() - indexStartedAt
		}

		const dependStartedAt = performance.now()
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddDependMS += performance.now() - dependStartedAt
		}
		
		const progressStartedAt = performance.now()
		this.#queueLoadingProgress()
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddProgressMS += performance.now() - progressStartedAt
		}
		if ( this.#scanStats !== null ) {
			this.#scanStats.modsAddedMS += performance.now() - startedAt
		}
	}

	#queueLoadingProgress(amount = 1) {
		this.#loadingProgressPending += amount
		if ( this.#loadingProgressPending >= 25 ) { this.#flushLoadingProgress() }
	}

	#flushLoadingProgress() {
		if ( this.#loadingProgressPending === 0 ) { return }
		this.#loadingWindow.current(this.#loadingProgressPending)
		if ( this.#scanStats !== null ) { this.#scanStats.progressUpdates++ }
		this.#loadingProgressPending = 0
	}
	...
	async #addMod(collectKey, thisFile) {
		if ( this.#scanStats !== null ) { this.#scanStats.totalFiles++ }
		const fileStatsStartedAt = performance.now()
		const thisFileStats = this.#fileStats(collectKey, thisFile)
		if ( this.#scanStats !== null ) {
			this.#scanStats.fileStatsMS += performance.now() - fileStatsStartedAt
		}
		...
		if ( this.#scanStats !== null ) {
			this.#scanStats.cacheLookupMS += performance.now() - cacheLookupStartedAt
		}
		if ( modInCache ) {
			if ( this.#scanStats !== null ) { this.#scanStats.cacheHits++ }
			// this.#log.debug(`mod-${modInCache.uuid}`, 'Adding mod FROM cache', modInCache.fileDetail.shortName)
			this.#addModToData(collectKey, {
				...modInCache,
				fileDetail : {
					...modInCache.fileDetail,
					fileDate : (this.#useSyncSafeMode) ? thisFileStats.birthDate.toISOString() : thisFileStats.date.toISOString(),
					fileSize : thisFileStats.size,
					fullPath : thisFileStats.fullPath,
		...
		if ( this.#scanStats !== null ) { this.#scanStats.cacheMisses++ }
		this.#ensureThreads(this.#scanStats?.cacheMisses ?? 1)
