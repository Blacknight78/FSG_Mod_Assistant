...
class modFileCollection {
	...
	modHubFullRecord(thisMod) {
		const gameVersion = thisMod.gameVersion || this.#util_gameVersion(thisMod.modDesc?.descVersion)
		const modHubID    = this.modHubModRecord(thisMod, gameVersion)
		const gameList    = this.#modHubList.games?.[gameVersion]

		return {
			id      : modHubID,
			version : this.modHubVersionModHubId(modHubID, gameVersion),
			recent  : (gameList?.last || this.#modHubList.last).includes(parseInt(modHubID)),
		}
	}
	 ...
	 * @returns {?number}
	 */
	modHubModRecord(thisMod, gameVersion = null) {
		const shortName   = thisMod.fileDetail.shortName
		const thisVersion = gameVersion || thisMod.gameVersion || this.#util_gameVersion(thisMod.modDesc?.descVersion)
		const gameMods    = this.#modHubList.games?.[thisVersion]?.mods

		if ( gameMods && Object.hasOwn(gameMods, shortName) ) {
			return gameMods[shortName]
		}
		if ( thisVersion !== 25 && Object.hasOwn(this.#modHubList.legacy || {}, shortName) ) {
			return this.#modHubList.legacy[shortName]
		}
		return this.#modHubList.games ? null : (this.#modHubList.mods[shortName] || null)
	}
	...
	modHubModUUID(modUUID) {
		const shortName   = this.#map_ModUUIDToShortName[modUUID]
		const gameVersion = this.#map_ModUUIDToGameVersion[modUUID]
		return this.modHubModRecord({ fileDetail : { shortName }, gameVersion }, gameVersion)
	}
	...
	modHubVersionUUID(modUUID) {
		return this.modHubVersionModHubId(
			this.modHubModUUID(modUUID),
			this.#map_ModUUIDToGameVersion[modUUID]
		)
	}
	...
	modHubVersionModRecord(thisMod) {
		const gameVersion = thisMod.gameVersion || this.#util_gameVersion(thisMod.modDesc?.descVersion)
		return this.modHubVersionModHubId(this.modHubModRecord(thisMod, gameVersion), gameVersion)
	}
	 ...
	 * @returns {?string}
	 */
	modHubVersionModHubId(modHubID, gameVersion = null) {
		const gameVersions = this.#modHubVersion.games?.[gameVersion]

		if ( gameVersions && Object.hasOwn(gameVersions, modHubID) ) {
			return gameVersions[modHubID]
		}
		if ( gameVersion !== 25 && Object.hasOwn(this.#modHubVersion.legacy || {}, modHubID) ) {
			return this.#modHubVersion.legacy[modHubID]
		}
		return this.#modHubVersion.games ? null : (this.#modHubVersion[modHubID] || null)
	}
	...
	#addModToData(collectKey, modRecord) {
		...
		this.#map_ModUUIDToGameVersion[thisModRecord.uuid] = thisModRecord.gameVersion
