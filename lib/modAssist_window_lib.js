...
module.exports.windowLib = class {
	...
	winDefs = {
		...
		mod_management : {
			callback        : () => { return },
			HTMLFile        : 'mod_management.html',
			refocusCallback : true,
			subWindowArgs   : { preload : 'sharedWindow' },
			winName         : 'mod_management',
		},
		vault_update : {
			callback        : () => { return },
			HTMLFile        : 'vault_update.html',
			refocusCallback : true,
			subWindowArgs   : { preload : 'sharedWindow' },
			winName         : 'vault_update',
		},
		recent_changes : {
			callback        : () => { return },
			HTMLFile        : 'recent_changes.html',
			refocusCallback : true,
			subWindowArgs   : { preload : 'sharedWindow' },
			winName         : 'recent_changes',
		},
		change : { //done
			callback        : () => { return },
			handleURLinWin  : true,
	...
	/**
	 * Get taskbar/window title label
	 * MARK: getWindowTitle
	 * @param {string} winName window
	 * @param {?string} title supplied title
	 * @returns {{label: string, lock: boolean}} title info
	 */
	#getWindowTitle(winName, title = null) {
		if ( title !== null ) {
			return { label : title, lock : true }
		}

		const appName = this.l10n.syncStringLookup('app_name')
		const windowLabels = {
			basegame19     : 'Base Game FS19',
			basegame22     : 'Base Game FS22',
			basegame25     : 'Base Game FS25',
			backups        : 'Backups',
			change         : 'Changelog',
			compare19      : 'Compare FS19',
			compare22      : 'Compare FS22',
			compare25      : 'Compare FS25',
			debug          : 'Debug',
			detail19       : 'Details FS19',
			detail22       : 'Details FS22',
			detail25       : 'Details FS25',
			find           : 'Find',
			gamelog        : 'Game Log',
			history        : 'History',
			...
			notes          : 'Notes',
			recent_changes : 'Collection Updates',
			resolve        : 'Resolve',
			save           : 'Savegame',
			save_manage    : 'Save Manager',
			save_track     : 'Save Tracker',
			setup          : 'Setup',
			update         : 'Update Candidates',
			vault          : 'Vault',
			vault_update   : 'Vault Updates',
			version        : 'Version',
		}

		return Object.hasOwn(windowLabels, winName) ?
			{ label : `FSG-${windowLabels[winName]}`, lock : true } :
			{ label : appName, lock : false }
	}
	...
	getRealCenter (name) {
		...
		const winSettings = this.#settings.get(`wins.${name}`) ?? { h : 600, m : false, w : 900, x : -1, y : -1 }
	...
	checkValidPlacement(winSettings) {
		const newWinSettings = winSettings ?? { h : 600, m : false, w : 900, x : -1, y : -1 }
	...
	createNamedMulti(winName, windowArgs, extraCallback = null) {
		...
		if ( thisWinName.startsWith('detail') ) {
			thisWindow.on('close', (event) => {
				...
				setTimeout(() => {
					if ( !thisWindow.isDestroyed() ) { thisWindow.destroy() }
				}, 50)
			})
		}
	...
	createSubWindow(winName, { sizeable = true, noChrome = false, useCustomTitle = true, skipTaskbar = false, noSelect = true, show = true, parent = null, title = null, fixed = false, frame = true, move = true, preload = null, fixedOnTop = true} = {}) {
		...
		const winTitle    = this.#getWindowTitle(winName, title)
		...
		if ( winTitle.lock ) {
			thisWindow.on('page-title-updated', (event) => {
				event.preventDefault()
				thisWindow.setTitle(winTitle.label)
			})
		}
	...
	createMainWindow (openCallback, doSplashScreen = true) {
		const mainWindowStartedAt = performance.now()
		const splashHandoffDelayMS = 250

		this.win.main = this.createSubWindow('main', { noSelect : false, show : !doSplashScreen, preload : 'sharedWindow' })
		serveIPC.log.info('performance', `Main window shell created in ${(performance.now() - mainWindowStartedAt).toFixed(1)} ms`)
		...
		if ( doSplashScreen ) {
			...
			serveIPC.log.info('performance', `Splash window load requested in ${(performance.now() - splashStartedAt).toFixed(1)} ms`)

			this.win.splash.on('closed', () => { this.win.splash = null })
		}

		const mainLoadStartedAt = performance.now()
		...
		this.win.main.webContents.on('did-finish-load', () => {
			serveIPC.log.info('performance', `Main window did-finish-load in ${(performance.now() - mainLoadStartedAt).toFixed(1)} ms`)
			if ( doSplashScreen ) {
				serveIPC.log.info('performance', `Main window splash handoff delay ${splashHandoffDelayMS} ms`)
				setTimeout(() => {
					this.win.main.show()
					this.safeClose('splash')
					this.destroyAndFocus('splash')
				}, splashHandoffDelayMS)
			}
			const showCount = setInterval(() => {
				if ( this.isVisible('main') ) {
					clearInterval(showCount)
					serveIPC.log.info('performance', `Main window visible in ${(performance.now() - mainWindowStartedAt).toFixed(1)} ms`)
					openCallback()
...
module.exports.defaultSettings = class {
	...
	get defaults () { return {
		...
		wins              : { type : 'object', default : {}, properties : {
			...
			backups       : this.#winDef(1000, 650),
			manifest      : this.#winDef(1000, 650),
			mod_management : this.#winDef(900, 600),
			vault_update   : this.#winDef(1000, 700),
			recent_changes : this.#winDef(1000, 650),
